...
RSpec.describe "FollowerFollowedモデルのテスト", type: :model do
  ...
  context "バリデーションのテスト" do
    ...
    it "follower_id,followed_idのいずれかがnilの場合はエラーとなること" do
      ...
      @follower_user.save
      follower_followed = @follower_user.active_relationships.first
    ...
    it "同じfollower_id,followed_idの組み合わせを持つレコードの場合はバリデーションエラーとなること" do
      ...
      @follower_user.follow(@followed_user)
      @follower_user.save
      follower_user_second.follow(followed_user_second)
      follower_user_second.save

      # 同じfollower_id,followed_idの組み合わせの場合はエラーとなること
      expect(
        @follower_user.active_relationships.build(followed_id: @followed_user.id)
      ).not_to be_valid
      expect(
        follower_user_second.active_relationships.build(followed_id: followed_user_second.id)
      ).not_to be_valid

      # 異なる組み合わせの同じfollower_id,followed_idの場合はエラーとならないこと
      expect(
        @follower_user.active_relationships.build(followed_id: followed_user_second.id)
      ).to be_valid
      expect(
        follower_user_second.active_relationships.build(followed_id: @followed_user.id)
      ).to be_valid
    end
  end
